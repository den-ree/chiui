...
Bindify is a lightweight, type-safe state management library that provides unidirectional data flow with flexible side effect handling. It combines Swift's actor model with Combine to deliver predictable state management and reactive UI updates.

## Core Components

### State Management

- ``BindifyStore`` - Thread-safe actor-based store for application state
- ``BindifyState`` - Base protocol for all state types
- ``BindifyStoreState`` - Protocol for global store state
- ``BindifyViewState`` - Protocol for local view state

### View Integration

- ``BindifyView`` - SwiftUI view wrapper with automatic store integration
- ``BindifyViewModel`` - Reactive view model with flexible side effects
- ``BindifyContext`` - Environment context for store access
- ``BindifyStateSideEffect`` - Chainable side effect system

## Key Features

### Flexible Side Effects

Bindify's `then()` API provides powerful chaining for state updates:
...
// Local view state
struct ProfileViewState: BindifyViewState {
...
```swift
// Create store
let store = BindifyStore(AppStoreState(settings: .default))
...
```swift
final class ProfileViewModel: BindifyViewModel<AppContext, ProfileViewState> {
